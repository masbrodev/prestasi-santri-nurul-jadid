                ...
                <form action="{{ URL::to('edit-peminatan/'.$r->id)}}" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select name="jurusan" class="form-control">
                            <option value="Sastra/Drama" {{ $r->jurusan == 'Sastra/Drama' ? 'selected':'' }}>Sastra/Drama</option>
                            <option value="Publik Speaking/Broadcasting" {{ $r->jurusan == 'Publik Speaking/Broadcasting' ? 'selected':'' }}>Publik Speaking/Broadcasting</option>
                            <option value="Jurnalistik" {{ $r->jurusan == 'Jurnalistik' ? 'selected':'' }}>Jurnalistik</option>
                            <option value="Musik" {{ $r->jurusan == 'Musik' ? 'selected':'' }}>Musik</option>
                            <option value="Olahraga" {{ $r->jurusan == 'Olahraga' ? 'selected':'' }}>Olahraga</option>
                            <option value="Seni Gerak" {{ $r->jurusan == 'Seni Gerak' ? 'selected':'' }}>Seni Gerak</option>
                            <option value="Seni Rupa" {{ $r->jurusan == 'Seni Rupa' ? 'selected':'' }}>Seni Rupa</option>
...
<div class="modal fade" id="tambah-peminatan">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Tambah Peminatan Skill</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ URL::to('tambah-peminatan')}}" method="post">
                    @csrf
                    <div class="form-group">
                        <input type="hidden" value="Skill" name="nama">
                        <label>Jurusan Peminatan</label>
                        <select name="jurusan" class="form-control">
                            <option value="Sastra/Drama">Sastra/Drama</option>
                            <option value="Publik Speaking/Broadcasting">Publik Speaking/Broadcasting</option>
